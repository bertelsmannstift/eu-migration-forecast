...
def get_language_id(language):
    with db.get_session() as session:
        return (
            pd.read_sql(
                select(db.Language).filter(db.Language.short == language), session.bind
            )["id"]
            .values[0]
            .item()
        )
...
def get_version_id(version):
    with db.get_session() as session:
        return (
            pd.read_sql(
                select(db.Version).filter(db.Version.version == version), session.bind
            )["id"]
            .values[0]
            .item()
        )
...
for language in languages:
    ...
    keywords.loc[:, ("FlagWithoutGermany")] = keywords.apply(
        lambda row: 0 if row.isna()["FlagWithoutGermany"] else 1, axis=1
    ).astype("bool")
    ...
    keywords = keywords[
        ["keyword_id", "language_id", "version_id", "without_germany", "keyword"]
    ]
    keywords.to_sql(
        "l_keyword",
        db.engine,
